...
\[
\begin{aligned}
&\texttt{frontier = \{source\};}\\
&\texttt{visited[source] = true;}\\
&\texttt{dist[source] = 0;}\\
&\texttt{level = 0;}\\
&\texttt{while (!frontier.empty()) \{}\\
&\qquad \texttt{next\_frontier.clear();}\\
&\qquad \texttt{\#pragma omp parallel}\\
&\qquad \texttt{\{}\\
&\qquad\qquad \texttt{std::vector<int> local\_next;}\\
&\qquad\qquad \texttt{\#pragma omp for schedule(dynamic,1) nowait}\\
&\qquad\qquad \texttt{for (int i = 0; i < frontier.size(); ++i) \{}\\
&\qquad\qquad\qquad \texttt{int u = frontier[i];}\\
&\qquad\qquad\qquad \texttt{for (int v : adj[u]) \{}\\
&\qquad\qquad\qquad\qquad \texttt{bool expected = false;}\\
&\qquad\qquad\qquad\qquad \texttt{if (visited[v].compare\_exchange\_strong(}\allowbreak\\
&\qquad\qquad\qquad\qquad\qquad \texttt{expected, true)) \{}
&\qquad\qquad\qquad\qquad\qquad \texttt{dist[v] = level + 1;}\\
&\qquad\qquad\qquad\qquad\qquad \texttt{local\_next.push\_back(v);}\\
&\qquad\qquad\qquad\qquad \texttt{\}}\\
&\qquad\qquad\qquad \texttt{\}}\\
&\qquad\qquad \texttt{\}}\\
&\qquad\qquad \texttt{\#pragma omp critical}\\
&\qquad\qquad \texttt{next\_frontier.insert(next\_frontier.end(),}\\
&\qquad\qquad \texttt{local\_next.begin(), local\_next.end());}
&\qquad \texttt{\}}\\
&\qquad \texttt{frontier.swap(next\_frontier);}\\
&\qquad \texttt{++level;}\\
&\texttt{\}}
\end{aligned}
\]
